...
static int mp2762_create_votable(struct mp2762 *chip)
{
	...
	chip->enable_votable = create_votable("BBC_ENABLE", VOTE_SET_ANY, mp2762_enable_vote_callback, chip);
...
static void mp2762_parse_cmdline(void)
{
	...
	mps_info("pissarro = %d, pissarropro = %d, pissarroinpro = %d\n", pissarro ? 1 : 0, pissarropro ? 1 : 0, pissarroinpro ? 1 : 0);
...
static int mp2762_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (product_name == PISSARROPRO) {
		...
		mps_info("PISSARRO no need to probe MP2762\n");
		return -ENODEV;
	}
	...
	chip->chip_ok = true;
	mps_info("MP27662 probe success\n");
	return ret;
}
