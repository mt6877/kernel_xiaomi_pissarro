...
#include "../mediatek/charger_pissarro/mtk_charger_init.h"
#include "../mediatek/charger_pissarro/mtk_charger_intf.h"
...
static void fg_read_volt(struct bq_fg_chip *bq)
{
	...
	if (bq->rw_lock) {
		if (bq->vbat == 0)
			bq->vbat = (product_name == PISSARRO) ? 4000 : 8000;
...
static void fg_update_status(struct bq_fg_chip *bq)
{
	...
	fg_info("%s [FG_STATUS] [UISOC RSOC RAWSOC TEMP_SOC SOH] = [%d %d %d %d %d], [VBAT CELL0 CELL1 IBAT TBAT] = [%d %d %d %d %d]\n", bq->log_tag,
		bq->ui_soc, bq->rsoc, bq->raw_soc, temp_soc, bq->soh, bq->vbat, bq->cell_voltage[0], bq->cell_voltage[1], bq->ibat, bq->tbat);
...
static int fg_check_device(struct bq_fg_chip *bq)
{
	...
	if (!strncmp(data, "bq27z561", 8) || !strncmp(data, "nfg1000a", 8) || !strncmp(data, "nfg1000b", 8)) {
...
static int fg_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	wakeup_source_init(&bq->gauge_wakelock, "gauge_wakelock");
