...
do {										\
	if (log_level >= 0)							\
		printk(KERN_ERR "[XMCHG_BQ25790] " fmt, ##__VA_ARGS__);	\
} while (0)
...
static int __bq25790_write_reg(struct bq25790 *bq, int reg, u8 val)
{
	...
	if (ret < 0) {
		bq_dbg("i2c write byte fail: can't write 0x%02X to reg 0x%02X\n",
				val, reg);
		return ret;
	}
...
static bool bq25790_check_usb_psy(struct bq25790 *bq)
{
	if (!bq->usb_psy)
...
static bool bq25790_check_xmusb350_psy(struct bq25790 *bq)
{
	if (!bq->xmusb350_psy)
		bq->xmusb350_psy = power_supply_get_by_name("xmusb350");
...
static int bq25790_set_revese_gpio(struct bq25790 *bq, bool enable)
{
	...
	if (ret)
		bq_dbg("failed to set reverse gpio dir ret:%d", ret);

	gpio_set_value(bq->reverse_gpio, val);

	bq_dbg("set reverse vbus to:%d, bq->reverse_gpio:%d \n", enable, bq->reverse_gpio);
	return ret;
}
...
static int bq25790_set_arti_vbus_disable(struct bq25790 *bq, bool disable)
{
	...
	if (ret)
		bq_dbg("failed to set gpio dir ret:%d", ret);
	...
	bq_dbg("set arti vbus disable to:%d   bq->arti_vbus_enable:%d\n", disable,  bq->arti_vbus_enable);
	return ret;
}
...
static int bq25790_tune_vbus_volt(bool up, int pulse)
{
	...
	if (!chg2_dev)
		chg2_dev = get_charger_by_name("xmusb350");
...
//	ret = charger_dev_tune_hvdcp_dpdm(chg2_dev, val);
	...
	bq_dbg("tune adapter voltage %s %s\n", up ? "up" : "down",
...
static int bq25790_raise_qc_vbus(struct bq25790 *bq, int num)
{
	...
	bq_dbg("bq25790_raise_qc_vbus start.\n");
	...
	while (bq->vbus_volt < 9000000) {
		...
		bq_dbg("vbus_volt:%d, tune_vbus_retry:%d.\n",
				bq->vbus_volt, tune_vbus_retry);

		if ((tune_vbus_retry > 230) && (bq->vbus_volt < 8400000)) {
			tune_vbus_retry = 0;
			bq_err("Failed to tune adapter volt into valid range\n");
			return false;
		}

		if (bq->vbus_volt < 4000000) {
			bq_dbg("Vbus is plug out.\n");
			return false;
		}
	}
	...
	if (bq->vbus_volt > 9500000) {
		...
		bq25790_update_status(bq);
		bq_dbg("DOWN:vbus_volt:%d.\n", bq->vbus_volt);
	}

	return true;
}

static int bq25790_set_charge_voltage(struct bq25790 *bq, u32 volt)
{
	int ret;
	u16 reg_val;
	...
	volt = 8900000;
	volt /= 1000;
	bq_err("ZLF:bq25790_set_charge_voltage:%d.\n", volt);
...
static int bq25790_get_charge_voltage(struct bq25790 *bq, u32 *volt)
{
	int ret;
	u16 reg_val;
...
static int bq25790_set_hiz_mode(struct bq25790 *bq, bool enable)
{
	...
	bq->hiz_mode = enable;
	bq_dbg("set hiz mode to enable:%d\n", enable);
...
static int bq25790_get_charge_current(struct bq25790 *bq, u32 *curr)
{
	int ret;
	u16 reg_val;
...
static int bq25790_get_input_current_limit(struct bq25790 *bq, u32 *curr)
{
	int ret;
	u16 reg_val;
...
static int bq25790_set_charge_current(struct bq25790 *bq, u32 curr)
{
	int ret;
	u16 reg_val;

	curr /= 1000;
	bq_err("ZLF:bq25790_set_charge_current:%d.\n", curr);
...
static int bq25790_set_input_volt_limit(struct bq25790 *bq, int volt)
{
	...
	volt /= 1000;
	bq_err("ZLF:bq25790_set_input_volt_limit:%d.\n", volt);
...
static int bq25790_set_input_curr_limit(struct bq25790 *bq, u32 curr)
{
	int ret;
	u16 reg_val;
	...
	curr /= 1000;
	bq_err("ZLF:bq25790_set_input_curr_limit:%d.\n", curr);
...
static int bq25790_set_term_current(struct bq25790 *bq, u32 curr)
{
	int ret;
	u8 reg_val;

	bq_info("set en_terminate = %dmA\n", curr);
...
static int bq25790_is_safety_timer_enabled(struct bq25790 *bq, bool *en)
{
	u8 reg_val;
	int ret = 0;
...
static int bq25790_charge_enable(struct bq25790 *bq, bool enable)
{
	...
	bq_err("ZLF:bq25790_charge_enable:%d.\n", enable);
...
static int bq25790_set_adc_enable(struct bq25790 *bq, int enable)
{
	...
	if (ret)
		bq_dbg("failed to set adc enable\n");
...
static int bq25790_read_bus_volt(struct bq25790 *bq, u32 *volt)
{
	int ret;
	int reg_val;
...
static int bq25790_read_bus_curr(struct bq25790 *bq, u32 *curr)
{
	int ret;
	int reg_val;
...
static int bq25790_read_bat_volt(struct bq25790 *bq, u32 *volt)
{
	int ret;
	int reg_val;
...
static int bq25790_read_bat_curr(struct bq25790 *bq, u32 *curr)
{
	int ret;
	s16 reg_val;
...
static int bq25790_get_mivr_state(struct bq25790 *bq, bool *in_loop)
{
	u8 status;
	int ret;
...
static int bq25790_is_power_path_enabled(struct bq25790 *bq, bool *enable)
{
	u8 status;
	int ret;
	...
	*enable = !!(status & BQ25790_CHARGE_EN_MASK);

	return 0;
}

static int bq25790_get_adc(struct bq25790 *bq, enum adc_channel chan, int *min, int *max)
{
	int channel;
	int ret = 0;
	u16 reg_val = 0;
...
static int bq25790_enable_otg(struct bq25790 *bq, bool enable)
{
	int ret;
	u8 reg_val;
...
static int bq25790_set_otg_current_limit(struct bq25790 *bq, u32 uA)
{
	int ret;
	u8 reg_val;
...
static int ops_bq25790_charge_enable(struct charger_device *chg_dev, bool enable)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_charge_enable(bq, enable);
}

static int ops_bq25790_set_charge_current(struct charger_device *chg_dev, u32 curr)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_set_charge_current(bq, curr);
}

static int ops_bq25790_get_charge_current(struct charger_device *chg_dev, u32 *curr)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_get_charge_current(bq, curr);
}

static int ops_bq25790_get_min_ichg(struct charger_device *chg_dev, u32 *curr)
{
	*curr = 300000;
...
static int ops_bq25790_set_charge_voltage(struct charger_device *chg_dev, u32 volt)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_set_charge_voltage(bq, volt);
}

static int ops_bq25790_get_charge_voltage(struct charger_device *chg_dev, u32 *volt)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_get_charge_voltage(bq, volt);
}

static int ops_bq25790_set_input_curr_limit(struct charger_device *chg_dev, u32 curr)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_set_input_curr_limit(bq, curr);
}

static int ops_bq25790_get_input_current_limit(struct charger_device *chg_dev, u32 *curr)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_get_input_current_limit(bq, curr);
}

static int ops_bq25790_get_min_aicr(struct charger_device *chg_dev, u32 *curr)
{
	*curr = 100000;
...
static int ops_bq25790_set_term_current(struct charger_device *chg_dev, u32 curr)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_set_term_current(bq, curr);
}

static int ops_bq25790_set_mivr(struct charger_device *chg_dev, u32 uV)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
...
static int ops_bq25790_get_mivr(struct charger_device *chg_dev, u32 *uV)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
...
static int ops_bq25790_get_mivr_state(struct charger_device *chg_dev, bool *in_loop)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_get_mivr_state(bq, in_loop);
}

static int ops_bq25790_set_term_enable(struct charger_device *chg_dev, bool enable)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_set_term_enable(bq, enable);
}

static int ops_bq25790_enable_power_path(struct charger_device *chg_dev, bool enable)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
...
static int ops_bq25790_is_power_path_enabled(struct charger_device *chg_dev, bool *enable)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_is_power_path_enabled(bq, enable);
}

static int ops_bq25790_enable_safety_timer(struct charger_device *chg_dev, bool enable)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_enable_safety_timer(bq, enable);
}

static int ops_bq25790_is_safety_timer_enabled(struct charger_device *chg_dev, bool *enable)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_is_safety_timer_enabled(bq, enable);
}

static int ops_bq25790_enable_otg(struct charger_device *chg_dev, bool enable)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_enable_otg(bq, enable);
}

static int ops_bq25790_set_otg_current_limit(struct charger_device *chg_dev, u32 uA)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_set_otg_current_limit(bq, uA);
}

static int ops_bq25790_get_adc(struct charger_device *chg_dev, enum adc_channel chan, int *min, int *max)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_get_adc(bq, chan, min, max);
}

static int ops_bq25790_read_bus_volt(struct charger_device *chg_dev, u32 *volt)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_read_bus_volt(bq, volt);
}

static int ops_bq25790_read_bus_curr(struct charger_device *chg_dev, u32 *curr)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_read_bus_curr(bq, curr);
}

static int ops_bq25790_read_bat_curr(struct charger_device *chg_dev, u32 *curr)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_read_bat_curr(bq, curr);
}

static int ops_bq25790_kick_wdt(struct charger_device *chg_dev)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
...
static int ops_bq25790_set_en_extilim(struct charger_device *chg_dev, bool enable)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_set_en_extilim(bq, enable);
}

static int ops_force_input_current_limit(struct charger_device *chg_dev, int value)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	bq->force_icl = value;
	if (bq->force_icl)
		bq25790_set_input_curr_limit(bq, bq->force_icl);

	return 0;
}

static const struct charger_ops bq25790_chg_ops = {
	.enable = ops_bq25790_charge_enable,
	.set_charging_current = ops_bq25790_set_charge_current,
	.get_charging_current = ops_bq25790_get_charge_current,
	.get_min_charging_current = ops_bq25790_get_min_ichg,
	.set_constant_voltage = ops_bq25790_set_charge_voltage,
	.get_constant_voltage = ops_bq25790_get_charge_voltage,
	.set_input_current = ops_bq25790_set_input_curr_limit,
	.get_input_current = ops_bq25790_get_input_current_limit,
	.get_min_input_current = ops_bq25790_get_min_aicr,
	.set_eoc_current = ops_bq25790_set_term_current,
	.set_mivr = ops_bq25790_set_mivr,
	.get_mivr = ops_bq25790_get_mivr,
	.get_mivr_state = ops_bq25790_get_mivr_state,
	.enable_termination = ops_bq25790_set_term_enable,
	.enable_powerpath = ops_bq25790_enable_power_path,
	.is_powerpath_enabled = ops_bq25790_is_power_path_enabled,
	.enable_safety_timer = ops_bq25790_enable_safety_timer,
	.is_safety_timer_enabled = ops_bq25790_is_safety_timer_enabled,
	.enable_otg = ops_bq25790_enable_otg,
	.set_boost_current_limit = ops_bq25790_set_otg_current_limit,
	.get_adc = ops_bq25790_get_adc,
	.get_vbus_adc = ops_bq25790_read_bus_volt,
	.get_ibus_adc = ops_bq25790_read_bus_curr,
	.get_ibat_adc = ops_bq25790_read_bat_curr,
	.kick_wdt = ops_bq25790_kick_wdt,
	.set_en_extilim = ops_bq25790_set_en_extilim,
	.force_input_current_limit = ops_force_input_current_limit,
};

static const struct charger_properties bq25790_chg_props = {
	.alias_name = "bbc",
};
...
static struct power_supply *get_bms_psy(struct bq25790 *bq)
{
	...
	if (!bq->bms_psy)
		bq_dbg("bms power supply not found\n");
...
static int bq25790_update_charging_profile(struct bq25790 *bq)
{
	...
	bq_info("charge_type:%d, iindpm:%d, fcc:%d\n", bq->psy_type, iindpm, fcc);
...
static int bq25790_parse_dt(struct bq25790 *bq)
{
	int ret;
	struct device_node *np = bq->dev->of_node;

	if (!np) {
		bq_err("device tree info missing\n");
		return -1;
	}

	ret = of_property_read_u32(np, "precharge-current", &bq->platform_data.iprechg);
	if (ret)
		bq_err("Failed to read node of precharge-current\n");

	ret = of_property_read_u32(np, "termi-curr", &bq->platform_data.iterm);
	if (ret)
		bq_err("Failed to read node of termi-curr\n");

	ret = of_property_read_u32(np, "safe_timer_en", &bq->platform_data.safe_timer_en);
	if (ret)
		bq_err("Failed to read node of safe_timer_en\n");

	ret = of_property_read_u32(np, "safe_timer", &bq->platform_data.safe_timer);
	if (ret)
		bq_err("Failed to read node of safe_timer\n");

	ret = of_property_read_u32(np, "presafe_timer", &bq->platform_data.presafe_timer);
	if (ret)
		bq_err("Failed to read node of presafe_timer\n");

	ret = of_property_read_u32(np, "vac_ovp", &bq->platform_data.vac_ovp);
	if (ret)
		bq_err("Failed to read node of vac_ovp\n");

	ret = of_property_read_u32(np, "cell_num", &bq->platform_data.cell_num);
	if (ret)
		bq_err("Failed to read node of cell_num\n");

	ret = of_property_read_u32(np, "vsys_min", &bq->platform_data.vsys_min);
	if (ret)
		bq_err("Failed to read node of ti,vsys_min\n");


	bq->platform_data.irq_gpio = of_get_named_gpio(np, "bq25790_irq_gpio", 0);
	if ((!gpio_is_valid(bq->platform_data.irq_gpio))) {
		bq_err("failed to parse xmusb350_irq_gpio\n");
		return -1;
	}
...
static int bq25790_detect_device(struct bq25790 *bq)
{
	...
	bq_dbg("%s:BQ25790_REG_PART_NUM:0x%x bq->part_no:0x%x bq->revision:%x",
...
static void bq25790_update_status(struct bq25790 *bq)
{
	...
	bq_dbg("vbus:%d,vbat:%d, ibus:%d,ichg:%d\n",
...
static void bq25790_fv_check(struct bq25790 *bq)
{
	...
	if ((status == POWER_SUPPLY_STATUS_CHARGING) && (vcell > fv_max) &&
		...
		bq_dbg("vcell:%d is high, reduce fv:%d\n", vcell,
	...
	bq_dbg("vcell:%d, fv:%d, fv_max:%d\n",
		vcell, get_effective_result(bq->fv_votable), fv_max);
}

static void bq25790_status_change_work(struct work_struct *work)
{
	struct bq25790 *bq = container_of(work, struct bq25790, charge_status_change_work.work);
	union power_supply_propval prop = {0,};
	int psy_type, hvdcp3_type = 0;
	bool hvdcp_flag = false;
	int ret;

	if (!bq25790_check_usb_psy(bq)) {
		bq_err("failed to init usb_psy");
		return;
	}

	if (!bq25790_check_xmusb350_psy(bq)) {
		bq_err("failed to init xmusb350_psy");
		return;
	}

	ret = power_supply_get_property(bq->xmusb350_psy, POWER_SUPPLY_PROP_HVDCP3_TYPE, &prop);
	if (ret < 0) {
		bq_err("failed to get hvdcp3_type\n", ret);
		return;
	}
	hvdcp3_type = prop.intval;

	ret = power_supply_get_property(bq->usb_psy, POWER_SUPPLY_PROP_REAL_TYPE, &prop);
	if (ret < 0) {
		bq_err("failed to get psy_type\n", ret);
		return;
	}
	psy_type = prop.intval;

	bq_dbg("psy_type:%d, bq->psy_type:%d.\n", psy_type, bq->psy_type);
	bq_dbg("hvdcp3_type:%d,  bq->hvdcp3_type:%d.\n", hvdcp3_type, bq->hvdcp3_type);

	if ((psy_type != POWER_SUPPLY_TYPE_UNKNOWN) && (bq->psy_type != psy_type || bq->hvdcp3_type != hvdcp3_type)) {
		bq->psy_type = psy_type;
		if ((bq->psy_type == POWER_SUPPLY_TYPE_USB_HVDCP)
				|| (bq->psy_type == POWER_SUPPLY_TYPE_USB_HVDCP_3)
				|| (bq->psy_type == POWER_SUPPLY_TYPE_USB_HVDCP_3_PLUS)) {
			...
			if (bq->psy_type == POWER_SUPPLY_TYPE_USB_HVDCP) {
				...
				if ((bq->psy_type == POWER_SUPPLY_TYPE_USB_HVDCP)
						&& ((psy_type == POWER_SUPPLY_TYPE_USB_HVDCP_3)
							|| (psy_type == POWER_SUPPLY_TYPE_USB_HVDCP_3_PLUS))) {
					ret = power_supply_get_property(bq->xmusb350_psy, POWER_SUPPLY_PROP_HVDCP3_TYPE, &prop);
					if (ret < 0) {
						bq_err("failed to get hvdcp3_type\n", ret);
						return;
					...
					hvdcp3_type = prop.intval;
					bq->psy_type = psy_type;
					bq_dbg("Update QC3 | QC3P5.\n");
				} else if ((bq->psy_type == POWER_SUPPLY_TYPE_USB_HVDCP)
						&& (psy_type == POWER_SUPPLY_TYPE_USB_HVDCP)) {
					bq->psy_type = psy_type;
					hvdcp_flag = true;
					bq_dbg("Keep QC2.\n");
				} else {
			...
			if ((bq->psy_type == POWER_SUPPLY_TYPE_USB_HVDCP_3)
					|| (bq->psy_type == POWER_SUPPLY_TYPE_USB_HVDCP_3_PLUS)
					|| hvdcp_flag) {

				if (bq->psy_type == POWER_SUPPLY_TYPE_USB_HVDCP_3_PLUS)
					ret = bq25790_raise_qc_vbus(bq, 10);
				else
					ret = bq25790_raise_qc_vbus(bq, 1);

				bq->hvdcp3_type = hvdcp3_type;
				bq25790_update_status(bq);
				bq_dbg("vbus_volt:%d.\n", bq->vbus_volt);

				if (bq->vbus_volt < 8800000) {
					bq->psy_type = POWER_SUPPLY_TYPE_USB_HVDCP;
					bq->hvdcp3_type = HVDCP3_NONE;
					bq_dbg("Raise vbus failed.\n");
				}
				bq25790_update_charging_profile(bq);
			}
		} else {
	...
	for (addr = 0x0; addr <= 0x48; addr++) {
		for (i = 0; i < 15; i++) {
			if (addr == bq25790_word_reg[i]) {
				...
				bq_dbg("Reg0x%.2x=0x%.4x\n", addr, word_val);
				goto next;
			}
		}
		bq25790_read_reg(bq, addr, &val);
next:
		bq_dbg("Reg0x%.2x=0x%.2x\n", addr, val);
		continue;
	}
}
...
static void bq25790_charge_irq_workfunc(struct work_struct *work)
{
	...
	if (!bq->power_good) {
		...
		bq_dbg("usb plug-out, set usb present = %d\n", bq->usb_present);
		bq->psy_type = POWER_SUPPLY_TYPE_UNKNOWN;
		bq->hvdcp3_type = HVDCP3_NONE;
		...
		bq->usb_present = true;
		bq_dbg("usb plug-in, set usb present = %d\n", bq->usb_present);
		ret = bq25790_init_device(bq);
		if (ret) {
			bq_dbg("Failed to init device\n");
			return;
		}
		...
		bq_dbg("usb plugged in, set usb present = %d\n", bq->usb_present);
	}
...
static irqreturn_t bq25790_interrupt(int irq, void *dev_id)
{
...
{
	bq25790_interrupt(bq->client->irq, bq);
}
		...
		int fcc_ua, const char *client)
{
	...
	bq_dbg("set fast charge current to :%d\n", fcc_ua);

	rc = bq25790_set_charge_current(bq, fcc_ua);
	if (rc < 0) {
		bq_dbg("failed to allocate register map\n");
		return rc;
	}
		...
		void *data, int disable, const char *client)
{
	...
	bq_dbg("client have set charging to %d\n", !disable);
		...
		unsigned long ev, void *v)
{
	...
	if (strcmp(psy->desc->name, "usb") == 0 || strcmp(psy->desc->name, "battery") == 0)
		schedule_delayed_work(&bq->charge_status_change_work, 0);

	return NOTIFY_OK;
}

static int bq25790_init_irq(struct bq25790 *bq)
{
	int ret = 0;

	ret = devm_gpio_request(bq->dev, bq->platform_data.irq_gpio, dev_name(bq->dev));
	if (ret < 0) {
		bq_err("failed to request gpio\n");
		return -1;
	}

	bq->platform_data.irq = gpio_to_irq(bq->platform_data.irq_gpio);
	if (bq->platform_data.irq < 0) {
		bq_err("failed to get gpio_irq\n");
		return -1;
	}

	ret = request_irq(bq->platform_data.irq, bq25790_interrupt, IRQF_TRIGGER_FALLING | IRQF_ONESHOT, dev_name(bq->dev), bq);
	if (ret < 0) {
		bq_err("failed to request irq\n");
		return -1;
	}

	enable_irq_wake(bq->platform_data.irq);

	return 0;
}

static void bq25790_parse_cmdline(void)
{
	...
	bq_info("pissarro = %d, pissarropro = %d, pissarroinpro = %d\n", pissarro ? 1 : 0, pissarropro ? 1 : 0, pissarroinpro ? 1 : 0);
				 ...
				 const struct i2c_device_id *id)
{
	...
	bq25790_parse_cmdline();

	if (product_name == PISSARROPRO) {
		bq_info("BQ25790 probe start\n");
	} else {
		bq_info("PISSARRO no need to probe BQ25790\n");
		return -ENODEV;
	}
	...
	ret = bq25790_init_irq(bq);
	if (ret) {
		bq_err("failed to init irq\n");
		return ret;
	}

	INIT_DELAYED_WORK(&bq->charge_irq_work, bq25790_charge_irq_workfunc);
	INIT_DELAYED_WORK(&bq->charge_monitor_work, bq25790_charge_monitor_workfunc);
	INIT_DELAYED_WORK(&bq->charge_status_change_work, bq25790_status_change_work);

	bq->chg_dev = charger_device_register("bbc", bq->dev, bq, &bq25790_chg_ops, &bq25790_chg_props);
	if (!bq->chg_dev) {
		bq_err("bq25790 charger device register fail\n");
		return ret;
	}
	...
	if (ret < 0) {
		bq_err("Couldn't register psy notifier rc = %d\n", ret);
		return ret;
	}
	...
	schedule_delayed_work(&bq->charge_irq_work, 0);
	bq_info("bq25790 probe successfully, Part Num:%d, Revision:0x%x \n", bq->part_no, bq->revision);
...
static int bq25790_resume(struct device *dev)
{
	...
	if (bq->irq_waiting) {
		...
		mutex_unlock(&bq->irq_complete);
		bq25790_interrupt(client->irq, bq);
	} else {
		mutex_unlock(&bq->irq_complete);
	}
